 ...
 *
 * @author Antoine Béland <abeland@ledevoir.com>
 */
...
export function scroller(callbacks) {
  ...
  const articleReminder = document.querySelector('.article-reminder');
  ...
  const _update = () => {
    ...
    // Display the title as a reminder when the main article title is hidden.
    const isArticleReminderDisplayed = title.getBoundingClientRect().bottom <= 0;
    const isArticleReminderIsVisible = articleReminder.classList.contains('visible');
    if (isArticleReminderDisplayed && !isArticleReminderIsVisible) {
      articleReminder.classList.add('visible')
    } else if (!isArticleReminderDisplayed && isArticleReminderIsVisible) {
      articleReminder.classList.remove('visible')
    }
  ...
  /**
   * Updates the scroll progression indicator.
   * @private
   */
  const _updateScrollProgress = () => {
