...
var getDistinct = function getDistinct(data, property) {
...
function getTopPlayers(data) {
  ...
  distincPlayer.forEach(function (p) {
    return countLine.push({
      name: p,
      count: 0
    });
  });
  data.map(function (line) {
    ...
    var pIdx = countLine.findIndex(function (element) {
  ...
  return countLine.map(function (d) {
    return d.name;
  }).slice(0, 5);
}
/**
...
function summarizeLines(data) {
  ...
  data.forEach(function (line) {
    var struct = structData.find(function (s) {
      return s.Act === line.Act;
    });
    ...
    if (struct === undefined) {
      ...
      var player = struct.Players.find(function (p) {
        return p.Player === line.Player;
      });
  ...
  data.forEach(function (act) {
    act.Players.forEach(function (p) {
      ...
      if (top.indexOf(p.Player) === -1) {
        var playerObj = act.Players.find(function (obj) {
          return obj.Player === 'Other';
        });
    ...
    act.Players = act.Players.filter(function (p) {
      return top.indexOf(p.Player) > -1 || p.Player === 'Other';
    });
  });
  return data;
}
...
function updateGroupXScale(scale, data, width) {
  ...
  scale.domain(groups).range([0, width]);
}
/**
...
function updateYScale(scale, data, height) {
  ...
  scale.domain([0, 105]).range([height, 0]);
}
/**
...
function createGroups(data, x) {
  d3.select('#graph-g').selectAll().data(data).enter().append('g').attr('transform', function (d) {
...
function drawBars(y, xSubgroup, players, height, color, tip) {
  // TODO : Draw the bars
  d3.select('#graph-g').selectAll('g.act').selectAll('rect').data(function (d) {
...
function appendGraphLabels(g) {
  g.append('text').text('Line Count').attr('class', 'y axis-text').attr('transform', 'rotate(-90)').attr('fill', '#898989').attr('font-size', 12);
  g.append('text').text('Lines per act').attr('class', 'title').attr('fill', '#898989');
}
/**
...
function updateXSubgroupScale(scale, players, xScale) {
...
function draw(data, color) {
  ...
  d3.select('div.legend').selectAll().data(data).enter().append('div').attr('class', 'legend-element').append('svg').attr('width', 30).attr('height', 15);
  d3.select('div.legend').selectAll('svg').append('rect').attr('width', 15).attr('height', 15).style('fill', function (d) {
    return color(d);
  });
  d3.select('div.legend').selectAll('.legend-element').append('text').text(function (d) {
    return d;
...
function getContents(d) {
  ...
  */
  var act = '<label id="tooltip-title">Act ' + d.Act + '</label><br><br>';
  var playerLabel = '<label style=font-weight: bold">Player: </label>';
  var playerValue = '<span class=tooltip-value>' + d.Player + '</span><br>';
  var countLabel = '<label style=font-weight: bold">Count: </label>';
  var countValue = '<span class=tooltip-value>' + d.Count + '</span><br>';
  return act + playerLabel + playerValue + countLabel + countValue;
}
...
(function (d3) {
  ...
  var barColors = ['#861388', '#d4a0a7', '#dbd053', '#1b998b', '#A0CED9', '#3e6680'];
  var bounds;
  var svgSize;
  var graphSize;
  ...
  d3.csv('./romeo_and_juliet.csv').then(function (data) {
    ...
    var topPlayers = preproc.getTopPlayers(data);
    ...
    var color = helper.defineColorScale(barColors, topPlayers);
...
},{"./scripts/preprocess.js":"scripts/preprocess.js","./scripts/viz.js":"scripts/viz.js","./scripts/helper.js":"scripts/helper.js","./scripts/legend.js":"scripts/legend.js","./scripts/tooltip.js":"scripts/tooltip.js","d3-tip":"node_modules/d3-tip/index.js"}],"node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
//# sourceMappingURL=/INF8808.e31bb0bc.js.map